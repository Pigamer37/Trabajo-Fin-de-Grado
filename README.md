...
> 
> It is necessary to launch the following command **on the Raspberry** too : `xhost +`<br>
...
`apt-get -y install \`<br>
    `firefox \`<br>
    `libcanberra-gtk-module \`<br>
    `libcanberra-gtk3-module`<br>
>
>Each line does the following, respectivelly: updates, installs OpenCV for developement, installs OpenCV for execution, installs colcon tools to compile on ROS, installs dependencies to make OpenCV be able to use displays.
...
>
> Es necesario **en la Raspberry** lanzar este comando también : `xhost +`<br>
