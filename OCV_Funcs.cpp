...
namespace LineProcess{
	...
	std::vector<cv::Point> GetHistFirstLanePoints(InputArray binImg, int bottomY, int topY, int laneX){
		...
		for(int i = 0; i<points.size(); i++){
			...
			if(pointDist>0){ //possible right lane
				// if(pointDist<=30){//too close to searchpoint
				// 	continue;
				// }
			...
			// else if(pointDist=0){//too close to searchpoint
			// 	continue;
			// }
		...
		//std::cout<<std::endl<<"leftLaneIdx: "<<leftLaneIdx<<", rightLaneIdx: "<<rightLaneIdx<<std::endl;
	...
	cv::Point GetHistNextLinePoint(InputArray binImg, int bottomY, int topY, int prevPointX, int thresh){
		...
		std::vector<cv::Point> points = GetHistLocalMaxes(binImg, bottomY, topY);
		//std::cout<<std::endl<<"number of Hist maxes: "<<points.size()<<std::endl;
		cv::Point lanePoint;
		...
		//std::cout<<std::endl<<"thresh: "<<thresh<<", prevPointX: "<<prevPointX<<std::endl;
		for(int i = 0; i<points.size(); i++){
			int pointDist = std::abs(points[i].x-prevPointX);
			//std::cout<<"laneIdx: "<<i<<", points[i].x: "<<points[i].x<<", pointDist: "<<pointDist<<std::endl;
        ...
        //std::cout<<"Final laneIdx: "<<laneIdx<<", points[laneIdx].x: "<<points[laneIdx].x<<std::endl;
		//cv::waitKey(-1);
		//cin.get();
		return lanePoint;
	}
