...
public:
    ...
    static std::vector<double> CastIntVecToDouble(const std::vector<int>& intVec) {
        std::vector<double> doubleVec;
        doubleVec.reserve(intVec.size()); // Reserve space to avoid multiple allocations

        for (int value : intVec) {
            doubleVec.push_back(static_cast<double>(value));
        }

        return doubleVec;
    }

    spline(const std::vector<int>& X, const std::vector<int>& Y,
           spline_type type = cspline,
           bool make_monotonic = false,
           bd_type left  = second_deriv, double left_value  = 0.0,
           bd_type right = second_deriv, double right_value = 0.0
        ...
        m_made_monotonic(false) // false correct here: make_monotonic() sets it
    {
        this->set_points(CastIntVecToDouble(X),CastIntVecToDouble(Y),m_type);
        if(make_monotonic) {
            this->make_monotonic();
        }
    }
